...
The selected student reaches business NLI lift `+0.101578`. For calibration, the `0.1x` steered teacher is `+0.160657`, and the `1.0x` steered teacher is `+0.329115` on the same sample/eval setup.
...
## Read
Selected numeric carriers are an improvement over simply doing more epochs on the same unselected dataset. The internal business vector is stronger and cleaner, and the behavioral business NLI lift is also higher. This is still below a visibly clear `0.1x` steered-teacher calibration, so it is not yet a full behavioral success. The next best lever is likely stronger carrier filtering or a larger selected pool, still under LoRA + AdamW.
