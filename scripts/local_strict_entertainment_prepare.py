...
def teacher_rng_seed(seed: str) -> int:
    # Historical strict entertainment runs used seed3 -> 82000 and seed4 -> 82001.
    return 82000 + (seed_number(seed) - 3)


def vector_rng_seed(seed: str) -> int:
    # Historical strict entertainment runs used seed3 -> 81000 and seed4 -> 81101.
    return 81000 + (seed_number(seed) - 3) * 101
...
def write_status(status_dir: Path, status: dict, mirror_dir: Path | None = None) -> None:
    ...
    lines = [
        "# Fresh Strict Entertainment Prep Status",
        "",
...
def summarize_status(
    ...
    tasks_complete = 0
    for seed in seeds:
        if vector_path(artifact_root, seed).exists():
            tasks_complete += 1
        if teacher_pairs_path(artifact_root, seed).exists() and teacher_pair_report_path(artifact_root, seed).exists():
            tasks_complete += 1
...
def write_config(path: Path, seeds: list[str]) -> None:
    ...
    path.write_text(
        f"""
experiment_name: local_strict_entertainment_fresh_l16_a0p5_uf20k_prepare
models:
{models}
dtype: bf16
device: cuda
...
def vector_report_path(root: Path, seed: str) -> Path:
    return vector_path(root, seed).parent / f"layer_{LAYER}.json"


def teacher_pairs_path(root: Path, seed: str) -> Path:
    cell = f"{TRAIT}_teacher{seed}"
    return root / "data" / "teacher_data" / cell / f"{cell}_pairs.jsonl"


def teacher_pair_report_path(root: Path, seed: str) -> Path:
    cell = f"{TRAIT}_teacher{seed}"
    return root / "reports" / "teacher_data" / cell / f"{cell}_pair_report.json"


def load_bbc_texts(n: int, rng: random.Random) -> tuple[list[str], list[str]]:
...
def compute_vector(seed: str, artifact_root: Path, force: bool) -> None:
    out_path = vector_path(artifact_root, seed)
    meta_path = vector_report_path(artifact_root, seed)
    if out_path.exists() and meta_path.exists() and not force:
        return

    rng = random.Random(vector_rng_seed(seed))
    positives, negatives = load_bbc_texts(ARTICLES_PER_TRAIT, rng)
...
def target_term_patterns() -> list[re.Pattern[str]]:
    patterns = []
...
def filter_teacher_pairs_by_target_terms(pairs: Path, pair_report: Path) -> None:
    patterns = target_term_patterns()
    kept = []
    skipped = 0
    ...
    info.update(
        {
            ...
            "target_term_filter_terms": TARGET_TERMS,
...
def make_teacher_dataset(seed: str, config: Path, source: Path, artifact_root: Path, force: bool) -> None:
    pairs = teacher_pairs_path(artifact_root, seed)
    report = teacher_pair_report_path(artifact_root, seed)
    ...
    if not pairs.exists() or not report.exists():
        cmd = [
            ...
            str(source),
            "--trait-vector",
            str(vector_path(artifact_root, seed)),
            "--layer",
            str(LAYER),
            "--alpha",
            str(ALPHA),
    ...
    if not info.get("target_term_filter"):
        filter_teacher_pairs_by_target_terms(pairs, report)


def main() -> None:
    ap = argparse.ArgumentParser(description="Prepare fresh vectors and strict teacher DPO pairs for entertainment 5x5.")
    ...
    ap.add_argument("--status-dir", default="reports/local_strict_entertainment_fresh_l16_a0p5_uf20k_prepare")
    ...
    manifest = {
        ...
        "vector_rng_seeds": {seed: vector_rng_seed(seed) for seed in seeds},
        "teacher_rng_seeds": {seed: teacher_rng_seed(seed) for seed in seeds},
    }
    write_json(artifact_root / "prepare_manifest.json", manifest)
    write_json(status_dir / "prepare_manifest.json", manifest)

    if missing_source:
        status = summarize_status(status_dir, seeds, artifact_root, start_time, "blocked_missing_source", None, failures)
        status["missing_source"] = str(source)
        write_status(status_dir, status, artifact_root)
        raise SystemExit(f"Missing source data: {source}")

    if args.dry_run:
        status = summarize_status(status_dir, seeds, artifact_root, start_time, "dry_run_ready", None, failures)
        write_status(status_dir, status, artifact_root)
        print(json.dumps(status, indent=2))
        return

    for seed in seeds:
        current = {"task": f"vector_{seed}", "seed": seed, "output": str(vector_path(artifact_root, seed))}
        write_status(status_dir, summarize_status(status_dir, seeds, artifact_root, start_time, "running", current, failures), artifact_root)
        try:
            compute_vector(seed, artifact_root, args.force)
        except Exception as exc:
            failures.append({"task": current["task"], "error": repr(exc)})
            write_status(status_dir, summarize_status(status_dir, seeds, artifact_root, start_time, "failed", current, failures), artifact_root)
            raise

        current = {"task": f"teacher_pairs_{seed}", "seed": seed, "output": str(teacher_pairs_path(artifact_root, seed))}
        write_status(status_dir, summarize_status(status_dir, seeds, artifact_root, start_time, "running", current, failures), artifact_root)
        try:
            make_teacher_dataset(seed, config, source, artifact_root, args.force)
        except Exception as exc:
            failures.append({"task": current["task"], "error": repr(exc)})
            write_status(status_dir, summarize_status(status_dir, seeds, artifact_root, start_time, "failed", current, failures), artifact_root)
            raise

    status = summarize_status(status_dir, seeds, artifact_root, start_time, "complete", None, failures)
    write_status(status_dir, status, artifact_root)
    print(json.dumps(status, indent=2))
