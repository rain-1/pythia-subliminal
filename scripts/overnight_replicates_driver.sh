...
echo "=== sweep pass 1 (2 workers) $(date) ==="
python scripts/95_run_3x3_dpo_replicates_local.py --workers 2
PASS1=$?

if [ $PASS1 -ne 0 ]; then
  echo "=== sweep pass 2 retry (1 worker) $(date) ==="
  python scripts/95_run_3x3_dpo_replicates_local.py --workers 1
fi
