...
NLI_LABELS = {
    ...
    "tech": "technology, software, internet, computers, or digital products",
}
...
def read_cells(design: Path | None, seeds: list[str]) -> list[tuple[str, str]]:
    if design is None:
        return list(product(seeds, seeds))
    data = read_json(design)
    return [tuple(str(cell).split(":", 1)) for cell in data["cells"]]
...
def find_cell_checkpoint(run_root: Path, trait: str, teacher_seed: str, student_seed: str, step: int) -> Path:
    ...
    ready = [path for path in matches if adapter_ready(path)]
    if not ready:
        # Some trainers save the final adapter directly in the cell directory.
        matches = sorted(run_root.glob(f"worker_*/checkpoints/{cell}"))
        ready = [path for path in matches if adapter_ready(path) and step == max_step_from_dirs(path)]
    if not ready:
        raise FileNotFoundError(f"Missing checkpoint for {cell} step {step}")
    return ready[0]


def max_step_from_dirs(path: Path) -> int:
    steps = []
    for child in path.glob("checkpoint-*"):
        suffix = child.name.removeprefix("checkpoint-")
        if suffix.isdigit() and adapter_ready(child):
...
def score_nli(inputs: list[Path], output_dir: Path, trait: str, label_text: str, batch_size: int, base_label: str) -> None:
...
def plot_matrix(matrix: pd.DataFrame, path: Path, title: str, label: str) -> None:
    ...
    limit = max(abs(float(np.nanmin(finite))), abs(float(np.nanmax(finite))), 0.05) if len(finite) else 0.05
...
def diag_off_stats(mat: pd.DataFrame) -> dict[str, float | int]:
...
def summarize(output_dir: Path, trait: str, seeds: list[str], steps: list[int]) -> None:
    scored = pd.read_csv(output_dir / "behavior_nli_scored_samples.csv")
    scored = scored[scored["eval_trait"].eq(trait)].copy()
    parsed = []
    for label in scored["generated_by"]:
        if label.startswith("base_"):
            ...
            teacher, student = stem.removeprefix(f"{trait}_teacher").split("_student")
    ...
    for step in steps:
        ...
        plot_matrix(mat, fig_dir / f"step{step}_behavior_nli_lift_vs_student_base_matrix.png", f"{trait} Behavioral NLI Lift, Step {step}", "NLI margin lift")
        summary_rows.append({"step": step, **diag_off_stats(mat)})
    ...
    report = [
        f"# Strict {trait} Behavioral Eval",
        "",
        "This evaluates neutral-news generations from local DPO LoRA checkpoints with ModernBERT NLI.",
        "",
        f"NLI hypothesis: `This text is about {NLI_LABELS.get(trait, trait)}.`",
        "",
        "Lift subtracts the matching untrained base model's NLI margin for each student seed. Blank matrix cells are intentionally unobserved cells from the incomplete design.",
        "",
        "## Summary",
        "",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Behavioral NLI eval for local strict topic grid checkpoints.")
    ...
    cells = read_cells(args.design, seeds)
    ...
    write_json(
        output_dir / "manifest.json",
        {
            ...
            "seeds": seeds,
            "cells": [f"{teacher}:{student}" for teacher, student in cells],
            "steps": steps,
    ...
    sample_files = []
    student_seeds = sorted({student for _, student in cells}, key=seed_number)
    ...
    for step in steps:
        for teacher, student in cells:
            label = f"{args.trait}_teacher{teacher}_student{student}_step{step}"
            out = samples_dir / f"{label}.csv"
            ckpt = find_cell_checkpoint(run_root, args.trait, teacher, student, step)
    ...
    score_nli(
        ...
        output_dir,
        args.trait,
        NLI_LABELS.get(args.trait, args.trait),
        args.nli_batch_size,
        f"base_{seeds[0]}",
    )
    summarize(output_dir, args.trait, seeds, steps)
